var AnalyticsPluginDatadogcom = function() {
    ...
    }, DOCS_ORIGIN = "https://docs.datadoghq.com", DOCS_TROUBLESHOOTING = `${DOCS_ORIGIN}/real_user_monitoring/browser/troubleshooting`;
    ...
    function callMonitored(fn, context, args) {
        try {
            ...
            if (monitor_displayIfDebugEnabled(e), onMonitorErrorCollected) ;
        }
    }
    ...
    const fileUrl = "((?:file|https?|blob|chrome-extension|electron|native|eval|webpack|snippet|<anonymous>|\\w+\\.|\\/).*?)", CHROME_LINE_RE = new RegExp(`^\\s*at (.*?) ?\\(${fileUrl}(?::(\\d+))?(?::(\\d+))?\\)?\\s*$`, "i"), CHROME_EVAL_RE = new RegExp("\\((\\S*)(?::(\\d+))(?::(\\d+))\\)");
    ...
    function initCookieStrategy(cookieOptions) {
        return {
            ...
            expireSession: sessionState => expireSessionCookie(cookieOptions)
        };
    }
    ...
    function expireSessionCookie(options, sessionState) {
        setCookie("_dd_s", toSessionString(getExpiredSessionState()), 144e5, options);
    }
    ...
    function startSessionStore(sessionStoreStrategyType, productKey, computeSessionState) {
        ...
        function synchronizeSession(sessionState) {
            return isSessionInExpiredState(sessionState) && (sessionState = getExpiredSessionState()), 
            ...
            }), sessionCache = sessionState) : (sessionCache = getExpiredSessionState(), expireObservable.notify())), 
            sessionState;
        }
        function startSession() {
            processSessionStoreOperations({
                process: sessionState => {
                    if (isSessionInNotStartedState(sessionState)) return getExpiredSessionState();
                },
                after: sessionState => {
        ...
        return {
            ...
            expire: () => {
                cancelExpandOrRenewSession(), expireSession(sessionCache), synchronizeSession(getExpiredSessionState());
            },
            stop: () => {
    ...
    const INTAKE_SITE_US1 = "datadoghq.com", PCI_INTAKE_HOST_US1 = "pci.browser-intake-datadoghq.com", INTAKE_URL_PARAMETERS = [ "ddsource", "ddtags" ];
    ...
    function startTracer(configuration, sessionManager) {
        return {
            ...
            traceFetch: context => injectHeadersIfTracingAllowed(),
            traceXhr: (context, xhr) => injectHeadersIfTracingAllowed()
        };
    }
    ...
    function startErrorCollection(lifeCycle, configuration, pageStateHistory, featureFlagContexts) {
        ...
        errorObservable.subscribe((error => lifeCycle.notify(13, {
            error: error
        }))), doStartErrorCollection(lifeCycle, pageStateHistory);
    }
    ...
    function startSessionManager(configuration, productKey, computeSessionState, trackingConsentState) {
        ...
        return sessionStore.renewObservable.subscribe((() => {
    ...
    function trackVisibility(configuration, expandSession) {
        ...
        addEventListener(configuration, document, "visibilitychange", expandSessionWhenVisible);
        timer_setInterval(expandSessionWhenVisible, 6e4);
    }
    ...
    const rawTelemetryEvent_types_TelemetryType = {
        ...
        usage: "usage"
    }, ALLOWED_FRAME_URLS = [ "https://www.datadoghq-browser-agent.com", "https://www.datad0g-browser-agent.com", "https://d3uc069fcn7uxw.cloudfront.net", "https://d20xtzwzcl0ceb.cloudfront.net", "http://localhost", "<anonymous>" ];
    let preStartTelemetryBuffer = boundedBuffer_createBoundedBuffer(), onRawTelemetryEventCollected = event => {
        preStartTelemetryBuffer.add((() => onRawTelemetryEventCollected()));
    };
    ...
    function startRum(configuration, recorderApi, customerDataTrackerManager, getCommonContext, initialViewOptions, createEncoder, trackingConsentState, customVitalsState) {
        ...
        const {addError: addError} = startErrorCollection(lifeCycle, configuration, pageStateHistory);
    ...
    function startRumEventCollection(lifeCycle, configuration, location, sessionManager, pageStateHistory, locationChangeObservable, domMutationObservable, windowOpenObservable, getCommonContext, reportError) {
        ...
        return startRumAssembly(configuration, lifeCycle, sessionManager, viewHistory, urlContexts, actionCollection.actionContexts, displayContext, ciVisibilityContext, getCommonContext, reportError), 
        {
            ...
            stop: () => {
                ciVisibilityContext.stop(), displayContext.stop(), urlContexts.stop(), viewHistory.stop(), 
    ...
    function datadog(settings) {
        ...
        __webpack_exports__datadogRum.init(options);
        function addAction(ctx) {
            return __awaiter(this, void 0, void 0, (function() {
                var event;
                return __generator(this, (function(_a) {
                    event = ctx.event;
                    __webpack_exports__datadogRum.addAction(event.event, event);
